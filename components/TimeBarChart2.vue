...
type Data = {
  dataKind: 'transition' | 'transition_ma' | 'cumulative'
  canvas: boolean
...
type Computed = {
  ...
  displayTransitionmaRatio: string
  displayInfo: {
    lText: string
    sText: string
    unit: string
...
> = {
  created() {
    ...
    this.dataKind =
      this.$route.query.embed && this.$route.query.dataKind === 'cumulative'
        ? 'cumulative' 
        : this.$route.query.embed && this.$route.query.dataKind === 'transition_ma'
        ? 'transition_ma'
        : 'transition'
  },
  ...
  computed: {
    ...
    displayTransitionmaRatio() {
      const lastDay = this.chartData.slice(-1)[0].transition_ma
      const lastDayBefore = this.chartData.slice(-2)[0].transition_ma
      return this.formatDayBeforeRatio(lastDay - lastDayBefore)
    },
    displayInfo() {
      ...
      if (this.dataKind === 'transition_ma') {
        return {
          lText: `${this.chartData.slice(-1)[0].transition_ma.toLocaleString()}`,
          sText: `${this.chartData.slice(-1)[0].label} ${this.$t(
            '実績値'
          )}（${this.$t('前日比')}: ${this.displayTransitionmaRatio} ${
            this.unit
          }）`,
          unit: this.unit
        }
    ...
    displayDataHeader() {
      ...
      if (this.dataKind === 'transition_ma') {
        return {
          ...
          datasets: [
            {
              data: [Math.max(...this.chartData.map(d => d.transition_ma))],
              backgroundColor: 'transparent',
              borderWidth: 0
            }
          ]
    ...
    scaledTicksYAxisMax() {
      const dataKind =
        this.dataKind === 'transition' ? 'transition'
            : this.dataKind === 'transition_ma' ? 'transition_ma'
            : 'cumulative'
